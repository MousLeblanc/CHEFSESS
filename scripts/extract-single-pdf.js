// scripts/extract-single-pdf.js
// Script pour extraire les recettes d'un seul PDF avec retours détaillés
import fs from 'fs';
...
async function extractSinglePDF(pdfFileName) {
  try {
    console.log('📚 Extraction d\'un seul PDF\n');
    ...
    if (recipes.length > 0) {
      ...
      console.log(`   ✅ ${outputPath.replace('.js', '.json')}`);
    } else {
    ...
    console.error('\n💡 Suggestions:');
    console.error('   - Vérifiez que le fichier PDF n\'est pas corrompu');
    console.error('   - Assurez-vous que pdf-parse est installé: npm install pdf-parse');
    console.error('   - Le PDF doit contenir du texte (pas seulement des images)');
...
if (!pdfFileName) {
  console.log('📚 Extraction de recettes depuis un PDF\n');
  console.log('Usage: node scripts/extract-single-pdf.js <nom-du-fichier.pdf>');
  console.log('\nExemple:');
  console.log('  node scripts/extract-single-pdf.js "_OceanofPDF.com_Simplissime_100_recettes_French_Edition_-_Jean-Francois_Mallet.pdf"');
  console.log('\n📋 Fichiers PDF disponibles:');
