// scripts/inject-arabian-magic-recipes-to-mongo.js
// Script pour injecter les recettes Arabian Magic dans MongoDB
...
function isValidRecipeName(name) {
  ...
  // Rejeter les noms par défaut
  if (name.match(/^Arabian Magic Recipe \d+$/i)) {
    return false;
  }
  
  // Rejeter les noms qui se terminent par un point (fragments de phrases)
  if (name.match(/\.$/)) {
    return false;
  }
  
  // Rejeter les noms qui commencent par des mots de description
  if (name.match(/^(The|When|This|There|One|Even|Variety|Culinary|Breakfasts|Snacks|Inns|King|Princess|Alibaba|Scheherazade|Marjina|Sindbad|Noor|Ul|Sabaah|Sindbad's|Marjina's|Scheherazade's|In a|Take|Put|Add|Mix|Combine|Blend|Preheat|Remove|Serve|Garnish|Cut|Make|Boil|Fry|Grind|Place|Brush|Wash|Pat|Roast|Discard|Chop|Season|Toss|Cover|Pour|Cool|Let|Stir|Heat|Melt|Bake|Remove|Cool|Serve|quenching|you get|rich seasoning|exclusively|versatile|salad with|inspired|form of|dish\.|Try it|Nights\.)/i)) {
    return false;
  }
  
  // Rejeter les noms trop longs (probablement des fragments de phrases)
  if (name.length > 80) {
    return false;
  }
  
  // Rejeter les noms qui contiennent des fragments de phrases évidents
  if (name.match(/(enough to|after sipping|to make you|without being|roasted lambs|with some|dishes or|to impress|is the refined|to see if|fits the taste|true taste|Arabian Nights\.)/i)) {
    return false;
  }
  
  // Rejeter les noms qui sont des instructions (commencent par des verbes)
  if (name.match(/^(Keep|Add|Put|Take|Mix|Combine|Blend|Preheat|Remove|Serve|Garnish|Cut|Make|Boil|Fry|Grind|Place|Brush|Wash|Pat|Roast|Discard|Chop|Season|Toss|Cover|Pour|Cool|Let|Stir|Heat|Melt|Bake|In a|Instructions|Ingredients|Mint sprig|Banana)/i)) {
    return false;
  }
 ...
 * Fonction principale
 */
async function injectArabianMagicRecipesToMongo(jsonPath) {
  try {
    ...
    if (newRecipes.length > 0) {
      ...
      console.log(`\n✅ ${inserted} recettes Arabian Magic injectées avec succès!`);
    } else {
...
if (import.meta.url === `file://${path.resolve(process.argv[1])}` || 
  ...
  const jsonPath = process.argv[2] || path.join(__dirname, '..', 'data', 'arabian-magic-recipes.json');
  
  injectArabianMagicRecipesToMongo(jsonPath)
    .catch(error => {
...
export default injectArabianMagicRecipesToMongo;
