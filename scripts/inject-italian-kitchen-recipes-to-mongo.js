// scripts/inject-italian-kitchen-recipes-to-mongo.js
// Script pour injecter les recettes Italian Kitchen dans MongoDB
 ...
 * Fonction principale
 */
async function injectItalianKitchenRecipesToMongo(jsonPath) {
  try {
    ...
    if (newRecipes.length > 0) {
      ...
      console.log(`\n✅ ${inserted} recettes Italian Kitchen injectées avec succès!`);
    } else {
...
if (import.meta.url === `file://${path.resolve(process.argv[1])}` || 
  ...
  const jsonPath = process.argv[2] || path.join(__dirname, '..', 'data', 'italian-kitchen-recipes.json');
  
  injectItalianKitchenRecipesToMongo(jsonPath)
    .catch(error => {
...
export default injectItalianKitchenRecipesToMongo;
