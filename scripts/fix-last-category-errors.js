// scripts/fix-last-category-errors.js
...
/**
 * Mappings des recettes à corriger depuis ACCOMPAGNEMENT vers PLAT
 */
const accompagnementToPlat = [
  ...
  'Purée de Pommes de Terre et Courgettes au Steak Haché',
  'Purée de pommes de Terre au jambon',
  'Purée de pommes de terre et jambon blanc',
  'Purée de pommes de terre et steak haché',
  'Rabbit With Mustard',
  'ZUCCHINI PASTA WITH BROCCOLI SPROUTS PESTO'
];

/**
 * Mappings des recettes à corriger depuis ACCOMPAGNEMENT vers PETIT-DÉJEUNER
 */
const accompagnementToPetitDejeuner = [
  'Mamma Rita\'s Eggs And Tomato Sauce'
];
 ...
 * Fonction principale
 */
async function fixLastCategoryErrors() {
  try {
    ...
    // Traiter les recettes depuis ACCOMPAGNEMENT vers PLAT
    console.log('🔍 Recherche des recettes à corriger depuis ACCOMPAGNEMENT vers PLAT...\n');
    for (const recipeName of accompagnementToPlat) {
      const recipe = await RecipeEnriched.findOne({ name: recipeName }).lean();
      
      if (recipe) {
        const currentCategory = recipe.category || 'accompagnement';
        
        if (currentCategory !== 'plat') {
          corrections.push({
            id: recipe._id,
            name: recipeName,
            oldCategory: currentCategory,
    ...
    // Traiter les recettes depuis ACCOMPAGNEMENT vers PETIT-DÉJEUNER
    console.log('🔍 Recherche des recettes à corriger depuis ACCOMPAGNEMENT vers PETIT-DÉJEUNER...\n');
    for (const recipeName of accompagnementToPetitDejeuner) {
      const recipe = await RecipeEnriched.findOne({ name: recipeName }).lean();
      
      if (recipe) {
        const currentCategory = recipe.category || 'accompagnement';
        
        if (currentCategory !== 'petit-déjeuner') {
          corrections.push({
            id: recipe._id,
            name: recipeName,
            oldCategory: currentCategory,
...
export default fixLastCategoryErrors;
