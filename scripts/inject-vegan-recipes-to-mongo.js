// scripts/inject-vegan-recipes-to-mongo.js
// Script pour injecter les recettes véganes dans MongoDB
 ...
 * Fonction principale
 */
async function injectVeganRecipesToMongo(jsonPath) {
  try {
    ...
    if (newRecipes.length > 0) {
      ...
      console.log(`\n✅ ${inserted} recettes véganes injectées avec succès!`);
    } else {
...
if (import.meta.url === `file://${path.resolve(process.argv[1])}` || 
  ...
  const jsonPath = process.argv[2] || path.join(__dirname, '..', 'data', 'vegan-recipes.json');
  
  injectVeganRecipesToMongo(jsonPath)
    .catch(error => {
...
export default injectVeganRecipesToMongo;
