// scripts/inject-world-recipes-to-mongo.js
// Script pour injecter les recettes du monde dans MongoDB
 ...
 * Fonction principale
 */
async function injectWorldRecipesToMongo(jsonPath) {
  try {
    ...
    if (newRecipes.length > 0) {
      ...
      console.log(`\n✅ ${inserted} recettes du monde injectées avec succès!`);
    } else {
...
if (import.meta.url === `file://${path.resolve(process.argv[1])}` || 
  ...
  const jsonPath = process.argv[2] || path.join(__dirname, '..', 'data', 'world-recipes.json');
  
  injectWorldRecipesToMongo(jsonPath)
    .catch(error => {
...
export default injectWorldRecipesToMongo;
